package com.example.renosyahputra.myjournal.dataCatatan.pengeluaran
...
class PengeluaranFrame : Fragment(), View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private fun addFragmentListByBulan(dataCatatan: ArrayList<CatatanData>, pager: ViewPager) {
        ...
        for (data in dataCatatan.sortedWith(compareBy(CatatanData::getTahun,CatatanData::getKodeBulan))) {
            val listCatatanFragment: ListCatatan = ListCatatan()
            listCatatanFragment.SetdataList(data.GetDetail())
            listCatatanFragment.SetCatatanType(TypeCatatan.pengeluaran)
